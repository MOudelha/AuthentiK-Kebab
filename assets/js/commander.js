...
function openOptionsModal(itemId, quickBuy = false) {
  ...
  body.addEventListener('change', (e) => {
    ...
    if (e.target.name === 'opt_crudites') {
      ...
      const maxCrud = getMenuData().crudites_max || 3;
      if (checked.length > maxCrud) e.target.checked = false;
    }
    // Limite sauces à 2
    if (e.target.name === 'opt_sauce') {
      const checked = body.querySelectorAll('[name="opt_sauce"]:checked');
      const maxSauce = getMenuData().sauces_max || 2;
      if (checked.length > maxSauce) e.target.checked = false;
    }
